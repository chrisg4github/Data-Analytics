...
select first_name || ' ' || last_name Actor_Name from actor limit 5;
...
select actor_id, first_name, last_name from actor where last_name ilike '%GEN%';
...
select last_name,first_name from actor where last_name ilike '%LI%' order by last_name, first_name;
